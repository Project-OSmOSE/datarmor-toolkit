...
def adjust_spectro(dataset: Spectrogram,
    ...
    if len(file_list) > 0:
        ...
        file_adjust = random.sample(
            get_audio_file(file_path=temp_adjustment_output_dir), number_adjustment_spectrogram
        )
...
def generate_spectro(dataset: Spectrogram,
    ...
    assert isinstance(datetime_begin, str), f"'datetime_begin' must be a datetime formatted string, {datetime_begin} not a valid value"
    assert isinstance(datetime_end, str), f"'datetime_end' must be a datetime formatted string, {datetime_end} not a valid value"
    ...
    if write_datasets_csv_for_APLOSE is True:
        ...
        dataset_info = {
                   ...
                   'file_type': file_type,
                   'identifier': dataset.campaign + '_' + dataset.name + '_' + str(dataset.spectro_duration) + '_' + str(dataset.dataset_sr)
        }
    ...
    for batch in range(dataset.batch_number):
        ...
        jobfile = dataset.jb.build_job_file(
                    script_path=Path(os.path.abspath('../src')).joinpath("qsub_spectrogram_generator_pkg.py"),
...
def display_progress(dataset: Spectrogram):
    ...
    assert isinstance(dataset, Spectrogram), "Not a Spectrogram object passed, display aborted"
